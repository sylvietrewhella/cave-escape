...
var
  saveTo: TextFile;
begin
  gData.HighestScore := gData.Score;
  Assign(saveTo, 'scorefile.txt');
  ReWrite(saveTo);
  WriteLn(saveTo, gData.HighestScore);
  Close(saveTo);
end;
...
begin
  ...
  begin
    if gData.Score > gData.HighestScore then
    begin
      SaveHighScore(gdata);
    end;
    ResetGame(gData);
...
begin
  ...
  begin
    ...
    DrawText(('HIGH SCORE ' + IntToStr(gData.HighestScore)),
    ColorWhite,
    'GameFont', ScreenWidth() / 2 - TextWidth(FontNamed('GameFont'),
    ('HIGH SCORE ' + IntToStr(gData.HighestScore))) / 2,
